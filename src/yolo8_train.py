...
from utils.yolo8_logger import Logger
...
def main():
    ...
    # Set missing attributes with default values if they do not exist
    if not hasattr(configs, 'working_dir'):
        configs.working_dir = '../'
    ...
    if configs.seed is not None:
        ...
        torch.backends.cudnn.benchmark = True  # Set benchmark to True for maximum GPU efficiency
...
def main_worker(gpu_idx, configs):
    ...
    # Set maximum memory usage fraction for each process (set to 95%)
    if configs.device.type == 'cuda':
        torch.cuda.set_per_process_memory_fraction(0.95, configs.gpu_idx)
...
def train_one_epoch(train_dataloader, model, optimizer, lr_scheduler, epoch, configs, logger, tb_writer):
    ...
    for batch_idx, batch_data in enumerate(tqdm(train_dataloader)):
        ...
        if global_step % configs.subdivisions == 0:
            ...
            # Clear CUDA cache after optimizer step to release unused memory
