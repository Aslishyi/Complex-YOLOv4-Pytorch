...
def create_optimizer(configs, model):
    ...
    if configs.optimizer_type == 'sgd':
        ...
        optimizer = torch.optim.AdamW(pg0, lr=configs.lr, weight_decay=configs.weight_decay)
    else:
        raise ValueError("Unknown optimizer type: {}".format(configs.optimizer_type))
...
def save_checkpoint(checkpoints_dir, saved_fn, model_state_dict, utils_state_dict, epoch):
    """Save checkpoint every epoch only if best model or after every checkpoint_freq epoch"""
...
if __name__ == '__main__':
    ...
    configs.lr_type = 'cosine'  # multi_step or cosine
